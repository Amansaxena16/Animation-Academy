      ...
      {steps.map((label, i) => {
        ...
        return (
          <li
            ...
            className={cn("flex min-w-0 flex-1 flex-col gap-2 text-[13px]", isCurrent ? "text-ink" : "text-ink-muted")}
          >
            <span aria-hidden className={cn("h-1 rounded-pill", done || isCurrent ? "bg-brand" : "bg-line")} />
            <span className="flex items-center gap-2 overflow-hidden font-semibold text-ellipsis whitespace-nowrap">
              <span
                ...
                className={cn(
                  ...
                  !done && !isCurrent && "border-line-strong bg-surface-raised text-ink-muted",
                )}
              >
                {done ? <Check className="size-3.5" strokeWidth={3} /> : i + 1}
              </span>
              <span className="sr-only">{done ? "Completed: " : isCurrent ? "Current: " : ""}</span>
              {label}
            </span>
          </li>
        );
